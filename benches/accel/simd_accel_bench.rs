...
fn bench_squared_l2(c: &mut Criterion) {
    ...
    for dim in [32, 128, 512, 1024] {
        ...
        group.bench_with_input(
            ...
            &dim,
            |b, _| b.iter(|| black_box(scalar.squared_l2_distance(black_box(left), black_box(right)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", dim),
            &dim,
            |b, _| b.iter(|| black_box(simd.squared_l2_distance(black_box(left), black_box(right)))),
        );
    }
    group.finish();
}
...
fn bench_inner_product(c: &mut Criterion) {
    ...
    for dim in [32, 128, 512, 1024] {
        ...
        group.bench_with_input(
            BenchmarkId::new("scalar", dim),
            &dim,
            |b, _| b.iter(|| black_box(scalar.inner_product_distance(black_box(left), black_box(right)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", dim),
            &dim,
            |b, _| b.iter(|| black_box(simd.inner_product_distance(black_box(left), black_box(right)))),
        );
    }
    group.finish();
}
...
fn bench_cosine_distance(c: &mut Criterion) {
    ...
    for dim in [32, 128, 512, 1024] {
        ...
        group.bench_with_input(
            BenchmarkId::new("scalar", dim),
            &dim,
            |b, _| b.iter(|| black_box(scalar.cosine_distance(black_box(left), black_box(right)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", dim),
            &dim,
            |b, _| b.iter(|| black_box(simd.cosine_distance(black_box(left), black_box(right)))),
        );
    }
    group.finish();
}
...
fn bench_batch_distance(c: &mut Criterion) {
    ...
    for batch_size in [10, 100, 1000] {
        ...
        group.bench_with_input(
            ...
            &batch_size,
            |b, _| b.iter(|| black_box(scalar.batch_squared_l2(black_box(query), black_box(&refs)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", batch_size),
            &batch_size,
            |b, _| b.iter(|| black_box(simd.batch_squared_l2(black_box(query), black_box(&refs)))),
        );
    }
    group.finish();
}
...
fn bench_key_compare(c: &mut Criterion) {
    ...
    for key_len in [8, 32, 128, 512] {
        ...
        group.bench_with_input(
            BenchmarkId::new("scalar", key_len),
            &key_len,
            |b, _| b.iter(|| black_box(scalar.compare_keys(black_box(&left), black_box(&right)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", key_len),
            &key_len,
            |b, _| b.iter(|| black_box(simd.compare_keys(black_box(&left), black_box(&right)))),
        );
    }
    group.finish();
}
...
fn bench_bloom_probe(c: &mut Criterion) {
    ...
    for num_keys in [10, 100, 1000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("scalar", num_keys),
            &num_keys,
            |b, _| b.iter(|| black_box(scalar.bloom_probe_batch(black_box(&filter), black_box(&refs)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", num_keys),
            &num_keys,
            |b, _| b.iter(|| black_box(simd.bloom_probe_batch(black_box(&filter), black_box(&refs)))),
        );
    }
    group.finish();
}
...
fn bench_columnar_u64_range(c: &mut Criterion) {
    ...
    for n in [1000, 10_000, 100_000] {
        ...
        group.bench_with_input(
            ...
            &n,
            |b, _| b.iter(|| black_box(scalar.columnar_u64_range_mask(black_box(&values), 100, 500))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", n),
            &n,
            |b, _| b.iter(|| black_box(simd.columnar_u64_range_mask(black_box(&values), 100, 500))),
        );
    }
    group.finish();
}
...
fn bench_columnar_f32_range(c: &mut Criterion) {
    ...
    for n in [1000, 10_000, 100_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("simd", n),
            &n,
            |b, _| b.iter(|| black_box(simd.columnar_f32_range_mask(black_box(&values), 100.0, 500.0))),
        );
    }
    group.finish();
}
...
fn bench_crc32c_batch(c: &mut Criterion) {
    ...
    for num_bufs in [10, 100, 1000] {
        let buffers: Vec<Vec<u8>> = (0..num_bufs)
            .map(|_| vec![0xBB_u8; 4096])
            .collect();
        let refs: Vec<&[u8]> = buffers.iter().map(Vec::as_slice).collect();

        group.bench_with_input(
            BenchmarkId::new("scalar", num_bufs),
            &num_bufs,
            |b, _| b.iter(|| black_box(scalar.crc32c_batch(black_box(&refs)))),
        );

        group.bench_with_input(
            BenchmarkId::new("simd", num_bufs),
            &num_bufs,
            |b, _| b.iter(|| black_box(simd.crc32c_batch(black_box(&refs)))),
        );
    }
    group.finish();
}
